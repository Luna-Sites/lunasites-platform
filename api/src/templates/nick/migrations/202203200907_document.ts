...
// Use env var or default for blobs directory (independent of Nick config)
const BLOBS_DIR = process.env.BLOBS_DIR || '/tmp/blobs';
...
export const down = async (knex: Knex): Promise<void> => {
  ...
  if (existsSync(BLOBS_DIR)) {
    readdirSync(BLOBS_DIR).forEach((file) =>
      rmSync(`${BLOBS_DIR}/${file}`),
    );
  }
};
